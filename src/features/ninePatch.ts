import {IPointData, ISize, NineSlicePlane, Texture, Container} from "pixi.js"
...
import {ClickListener} from "./clickable"
import {
    buttonClosePng,
...
function setupPanelOptions(panel: Container, options: PanelOptions): Container {
    const {position, size, label, scale} = options
...
export function panelAlert(options: PanelOptions): Container {
...
/** panel minimum size: (150, 100) */
export function panelBoosters(options: PanelOptions): Container {
    checkSize(options.size, {min: {width: 150, height: 100}})
    return setupPanelOptions(new NineSlicePlane(Texture.from(panelBoostersPng), 75, 50, 75, 50), options)
}
