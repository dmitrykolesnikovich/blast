...
import {animated, View} from "../engine"
import Image from "./views/Image"
import {panelInfo, Popup, popup} from "../features/ninePatch"
import Label from "./views/Label"
import Button from "./views/Button"
import Life from "./views/Life"
import {buttonRectangleBluePng, buttonRectangleShadowPng, coins1Png, life1Png, life3Png,} from "../../res"
import Navigation from "../features/navigation"
import {animatePulse} from "../features/animations"
...
export default class LivesShopDialog extends View<Layout> {
    ...
    constructor(navigation: Navigation) {
        ...
        this.layout = {
            ...
            descriptionIcon: animated(new Image({
        ...
        const {descriptionIcon} = this.layout
        animatePulse(descriptionIcon)
    }

    focused(controller: GameController) {
        const {model, navigation} = controller
        const {profile} = model
        const {life1, life2, life3, life4, life5, buttonRectangleBlue} = this.layout

        // lives
        life1.enabled = profile.lives > 0
        life2.enabled = profile.lives > 1
        life3.enabled = profile.lives > 2
        life4.enabled = profile.lives > 3
        life5.enabled = profile.lives > 4
        buttonRectangleBlue.enabled = profile.lives < 5

        //
        buttonRectangleBlue.layout.click = () => {
            profile.lives++
            navigation.update()
        }
    }
