...
export default class RadioButtonGroup extends Container {
    ...
    readonly layout: RadioButtonGroupLayout
    private selectedIndex: number
    private buttons: Array<RadioButton> = []

    constructor(layout: RadioButtonGroupLayout) {
        super()
        const {position, size, image, indicator, orientation, items, padding, style} = this.layout = layout
    ...
    select(selectedIndex: number) {
        ...
        if (this.layout.select) this.layout.select(this.layout.items[selectedIndex])
    }
